          ...
          {SKILLS.map((sk) => (
            <button key={sk} type="button"
              ...
              className={`px-3 py-1 rounded-full text-xs font-medium border transition-colors ${
                form.skills.includes(sk) ? 'bg-blue-600 border-blue-500 text-white' : 'bg-slate-800 border-slate-600 text-slate-300 hover:border-blue-500'}`}>
              {sk.replace('_', ' ')}
            </button>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {LANGUAGES.map((lang) => (
            <button key={lang} type="button"
              ...
              className={`px-3 py-1 rounded-full text-xs font-medium border transition-colors ${
                form.languages.includes(lang) ? 'bg-green-600 border-green-500 text-white' : 'bg-slate-800 border-slate-600 text-slate-300 hover:border-green-500'}`}>
              {lang}
            </button>
          ))}
        </div>
